...
- `runtime`: began decomposing `runtime/runtime_test.go` by ownership domain, moving cron and startup-recovery integration coverage into dedicated files (`runtime/cron_runtime_test.go`, `runtime/startup_recovery_runtime_test.go`) so runtime shell tests are less centralized in one monolithic test file.
- `runtime`: continued test-ownership decomposition by moving heartbeat and reflection integration coverage into `runtime/heartbeat_runtime_test.go`, shrinking `runtime/runtime_test.go` and keeping maintenance-loop behavior tests grouped by runtime shell concern.
- `runtime`: moved review-event delivery/generation and idle-expiry loop coverage into dedicated files (`runtime/review_runtime_test.go`, `runtime/idle_runtime_test.go`) so runtime shell behavior tests are grouped by concern instead of centralized in `runtime/runtime_test.go`.
- `runtime`: moved durable Telegram group integration coverage into `runtime/durable_group_runtime_test.go` so durable-child runtime behavior is tested in its own domain file instead of the monolithic runtime suite.
- `runtime`: continued domain extraction from `runtime/runtime_test.go` by moving backend/bootstrap (`runtime/backend_runtime_test.go`), tool-governance (`runtime/tools_runtime_test.go`), principal/entry-gate (`runtime/principal_runtime_test.go`), and face-render fallback (`runtime/face_render_runtime_test.go`) coverage into dedicated files.
- `runtime`: extracted voice/media/artifact inbound coverage (`runtime/media_artifact_runtime_test.go`) so transcription, voice synthesis fallback, media reply handling, and artifact-prep persistence tests live in one runtime-domain suite.
- `runtime`: extracted interactive brokerage/material/awareness integration coverage into `runtime/interactive_pipeline_runtime_test.go`, keeping negotiated execution-contract and material-floor fallback behavior out of the runtime test monolith.
