...
For recommendations, the current re-entry surface is a locally compliant
presentation and selection judgment: it can rank and suppress durable candidate
IDs, and it records the recommendation use it made. It is not yet the canonical
answer to "what is live now?" The Attention Radar direction moves that liveness
question into typed attention tracks and returns, so stale recommendations become
ghost tracks with provenance instead of fresh next-step advice reconstructed from
nearby state.
