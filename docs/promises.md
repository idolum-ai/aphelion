...
| Telegram media buttons for ambiguous voice/audio/video | implemented | `telegram.media.ambiguous_buttons` offers callback choices for audio/video processing, defaults stale/timeout prompts to agent-decide, and normalizes the approved choice into artifact metadata before routing. | Add provider-specific live probes for direct audio/video analysis quality if Gemini/native media use expands. |
