...
| Done-done roadmap completion | planned | A normative convergence plan now fixes operator surfaces to Telegram/CLI, preserves internal RPC for child control only, requires self-validation gates, and requires one final release with one live migration/init call. Tailnet now has local grant-binding projection and rollback evidence, but live child `tsnet` materialization and live Tailscale policy mutation remain outside the current implemented checkpoint. | Promote each remaining roadmap target into implemented code and tests without adding dashboard/web operator surfaces or partial releases. |
