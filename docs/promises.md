...
| String anonymization / provider-visible identity control | partial | `identity.user_agent` is configurable for provider clients and native `fetch_url`; anonymous profile suppresses Aphelion-specific default user agents unless the operator explicitly sets one. Cache boundaries are structural, but some model-visible names/tool descriptions remain project-specific by design. | Keep leak tests current and separate local relationship names from outbound protocol identity. |
