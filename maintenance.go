...
func runDurableAgentCommand(args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("durable-agent requires a subcommand: list, health, policy, enrollment, forensic, bootstrap, remote, or child-run")
	}
	...
	default:
		return fmt.Errorf("durable-agent subcommand must be one of list|health|policy|enrollment|forensic|bootstrap|remote|child-run")
	}
}
