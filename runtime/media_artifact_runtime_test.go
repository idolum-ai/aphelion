...
func TestHandleInboundMaterializesProviderImageGenerationMedia(t *testing.T) {
	...
	if !strings.HasPrefix(filepath.Clean(media.Path), filepath.Join(filepath.Clean(cfg.Agent.Workspace), "generated", "image-generation")) {
		t.Fatalf("media path = %q outside workspace generated root %q", media.Path, cfg.Agent.Workspace)
	}
