...
func (r *Runtime) sendContinuationApprovalPrompt(ctx context.Context, key session.SessionKey, msg core.InboundMessage, state session.ContinuationState, text string) error {
	...
	if messageID > 0 && r.store != nil {
		...
		if err := r.store.RecordTelegramCallbackMessage(msg.ChatID, messageID, msg.TelegramThreadID, continuationCallbackSurface, now); err != nil {
			return fmt.Errorf("record continuation callback message: %w", err)
		}
		r.retireStaleContinuationApprovalCards(ctx, key, msg.ChatID, messageID, "new_prompt", now)
	}
	return nil
}
...
func (r *Runtime) retireStaleContinuationApprovalCards(ctx context.Context, key session.SessionKey, chatID int64, keepMessageID int64, reason string, now time.Time) {
	...
	records, err := r.store.ListTelegramCallbackMessages(chatID, continuationCallbackSurface, now.Add(-continuationCardRetireWindow), continuationCardRetireLimit)
	if err != nil {
		r.recordContinuationCardRetirementEvent(key, chatID, 0, reason, "list_failed", err, now)
		log.Printf("WARN continuation card cleanup list failed chat_id=%d err=%v", chatID, err)
		return
	}
	for _, record := range records {
		...
		if err != nil && !telegramMessageNotModifiedError(err) {
			r.recordContinuationCardRetirementEvent(key, chatID, record.MessageID, reason, "edit_failed", err, now)
			log.Printf("WARN continuation card cleanup edit failed chat_id=%d message_id=%d err=%v", chatID, record.MessageID, err)
			continue
		}
		if markErr := r.store.RecordTelegramCallbackMessage(chatID, record.MessageID, record.ThreadID, continuationCallbackRetiredSurface, now); markErr != nil {
			r.recordContinuationCardRetirementEvent(key, chatID, record.MessageID, reason, "mark_failed", markErr, now)
			log.Printf("WARN continuation card cleanup mark failed chat_id=%d message_id=%d err=%v", chatID, record.MessageID, markErr)
			continue
		}
		r.recordContinuationCardRetirementEvent(key, chatID, record.MessageID, reason, "retired", nil, now)
	}
}

func (r *Runtime) recordContinuationCardRetirementEvent(key session.SessionKey, chatID int64, messageID int64, reason string, status string, err error, at time.Time) {
