...
func TestHandleTelegramCommandAutonomyAdmin(t *testing.T) {
	...
	if len(sender.inline) != 1 {
		t.Fatalf("inline = %#v, want one autonomy panel", sender.inline)
	}
...
func TestHandleTelegramCommandAutonomyPresetCallbackAppliesLeasedOverride(t *testing.T) {
	...
	handled, err := handleTelegramCommandCallback(context.Background(), sender, router, telegram.CallbackQuery{
		...
		Data: autonomyCallbackPrefix + "work15",
...
func TestHandleTelegramCommandAutoApproveNoArgsShowsPresetButtons(t *testing.T) {
	...
	sender := &stubCommandSender{}
	router := &stubCommandRouter{canRestart: true, autoApproveReturn: "Auto-approval status for this chat."}
	handled, err := handleTelegramCommand(context.Background(), sender, router, core.InboundMessage{
		ChatID:    7,
		SenderID:  1001,
		MessageID: 14,
	...
	if router.autoApproveChatID != 7 || router.autoApproveSenderID != 1001 || router.autoApproveArgs != "status" {
		t.Fatalf("autoapprove inputs = chat:%d sender:%d args:%q, want 7/1001/status", router.autoApproveChatID, router.autoApproveSenderID, router.autoApproveArgs)
	}
...
func TestHandleTelegramCommandAutoApprovePresetCallbackAppliesLease(t *testing.T) {
	...
	handled, err := handleTelegramCommandCallback(context.Background(), sender, router, telegram.CallbackQuery{
		...
		From: &telegram.User{ID: 1001},
		Data: autoApprovalCallbackPrefix + "deploy15",
		Message: &telegram.Message{
			MessageID: 77,
			Chat:      &telegram.Chat{ID: 7, Type: "private"},
		},
